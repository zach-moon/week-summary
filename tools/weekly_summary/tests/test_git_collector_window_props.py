...
# 年份范围：避开 32 位 time_t 与极端边界，确保 git / 日期换算稳定。
_MIN_YEAR = 2001
_MAX_YEAR = 2098
...
# min_codepoint=33 已排除空格与 ASCII 控制；这里再排除 C1 控制（Cc）、格式字符
# （Cf）、代理区（Cs）、行 / 段分隔符（Zl/Zp），保证 subject 经 git 规范化后保持
# 不变且非空白。
...
@settings(max_examples=100, deadline=None)  # git 子进程 I/O 较慢，关闭单例 deadline
...
def test_property_2_git_window_and_field_preservation(plan) -> None:
    ...
    窗内提交都应被采集（窗外提交不被采集）；返回的每条提交日期落在
    ``[start.date, end.date]`` 内；``repo_id``（仓库 basename）、``date``、``subject``
    三字段值与源一致。
    """
    window, specs = plan
